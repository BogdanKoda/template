...
class Module extends ServiceLocator
{
     ...
     */
    public function runAction($route, $params = [], $isCustomRoute = false)
    {
		
        // KODA NOT CODE $parts = $this->createController($route);
	    $isCustomRoute = $isCustomRoute ?? false;
	    $parts = $this->createController($route, $params, $isCustomRoute);
        if (is_array($parts)) {
     ...
     */
    public function createController($route, &$params = [], &$isCustomRoute = false)
	    // KODA NOT CODE public function createController($route)
    {
        if ($route === '') {
		...
		$isOpt = false;
        $oldRoute = $route;
		if((string) intval($route) === $route && $route != "") {
			$route = "";
			$isOpt = true;
		} else {
			$route = "/" . $route;
		}
	    $customRoute = ($_SERVER['REQUEST_METHOD'] ?? null) . "%|/" . $id . $route;
		
		if($isOpt) {
			$customRoute .= "%|OPT";
		}
        $routeFilePath = Yii::getAlias("@app") . "/components/Routing/web.php";
        require_once $routeFilePath;
	    $routeObject = Route::handle()->find($customRoute);
        if(!is_null($routeObject)) {
            $className = $routeObject['action'][0];
			if($isOpt) {
				$params[$routeObject['params'][1]] = intval($oldRoute);
			}

            if (is_subclass_of($className, 'yii\base\Controller')) {
                $action = $routeObject['action'][1];
                $id = explode("/", $id)[0] ?? $id;
                ...
                $controller = Yii::createObject($className, [[$id, $action], $this]);
                $returnController = get_class($controller) === $className ? $controller : null;
	            $isCustomRoute = true;
                return $returnController === null ? false : [$returnController, $action];
            } elseif (YII_DEBUG) {
                throw new InvalidConfigException('Controller class must extend from \\yii\\base\\Controller.');
            }
        }
	    if($isCustomRoute != 'console') {
		    $id = 'api';
	    }
        /*
         * END KODA EDIT
